...
def photo_thumb(slug, photo_id):
    """Serve a small cached preview without loading many originals into RAM."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
        return "Not found", 404
    ...
    # CRITICAL: acquire the semaphore BEFORE downloading the original.
    # Previously the download happened first, allowing dozens of full-size
    # wedding photos to occupy Render RAM at the same time and cause 503s.
    with THUMB_SEMAPHORE:
        # Another request may have generated it while this request waited.
        cached = thumb_cache_get(file_id)
        if cached is not None:
            return Response(
                cached,
        ...
        try:
            ...
            tg_url = f"{TG_FILE_API}/{file_path}"
            r = requests.get(tg_url, timeout=120)
            r.raise_for_status()

            img = Image.open(io.BytesIO(r.content))
            img = ImageOps.exif_transpose(img)

            # Keep the gallery preview light while preserving the original.
            ...
            # Never make the gallery blank just because Pillow cannot decode
            # one unusual image. Fall back to the original bytes.
            ...
            try:
                ...
                print(f"ERROR: photo fallback failed for {photo_id}: {fallback_error}")
                return "Photo unavailable", 502

            # Do not store a huge original in the thumbnail cache.
            return Response(
                cached,
        ...
        # Cache only the small generated preview.

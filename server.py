...
def photo_thumb(slug, photo_id):
    """Serve a small thumbnail without loading many full originals into RAM."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
        return "Not found", 404
    ...
    if cached is None:
        # IMPORTANT: acquire the semaphore BEFORE downloading the original.
        # Previously 68 browser requests could each download a full original
        # at once and exhaust Render RAM, causing HTTP 503.
        with THUMB_SEMAPHORE:
            ...
            if cached is None:
                ...
                # Use a temporary file instead of r.content so a 20-50MB
                # original is never duplicated in RAM.
                with tempfile.SpooledTemporaryFile(max_size=2 * 1024 * 1024, mode="w+b") as tmp:
                    ...
                    try:
                        ...
                        print(f"WARNING: thumbnail generation failed for {photo.get('filename')}: {e}")
                        # Safe fallback, but still read one request at a time.
                        tmp.seek(0)
                        cached = tmp.read()

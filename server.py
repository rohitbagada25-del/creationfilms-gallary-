...
def tg_send_bytes(data_bytes, filename, mimetype="image/jpeg", caption=""):
    """Upload generated image bytes to Telegram as an exact document."""
    files = {"document": (filename, io.BytesIO(data_bytes), mimetype)}
    data = {"chat_id": CHANNEL_ID, "caption": caption}
    r = requests.post(f"{TG_API}/sendDocument", data=data, files=files, timeout=120)
    r.raise_for_status()
    result = r.json()["result"]
...
def make_jpeg_variant(source_bytes, max_side, quality):
    """Create an EXIF-corrected JPEG variant at or below max_side."""
...
def admin_upload(slug):
    ...
    for f in uploaded:
        ...
        try:
            # Read once, then create the two display variants before uploading.
            source = f.stream.read()
            if not source:
                continue

            thumb_bytes = make_jpeg_variant(source, 1280, 82)
            preview_bytes = make_jpeg_variant(source, 2000, 76)

            # Keep the untouched original for full-resolution downloads.
            original_id, original_message_id = tg_send_bytes(
                source,
                secure_filename(f.filename) or "original.jpg",
                f.mimetype or "image/jpeg",
                caption=gallery["name"] + " | ORIGINAL"
            )
            thumb_id, thumb_message_id = tg_send_bytes(
                thumb_bytes,
                f"{uuid.uuid4().hex[:12]}_thumb.jpg",
                "image/jpeg",
                caption=gallery["name"] + " | THUMB"
            )
            preview_id, preview_message_id = tg_send_bytes(
                preview_bytes,
                f"{uuid.uuid4().hex[:12]}_preview.jpg",
                "image/jpeg",
                caption=gallery["name"] + " | PREVIEW"
            )
        except Exception as e:
        ...
        photo = {
            ...
            "thumb_message_id": thumb_message_id,
...
def photo_preview(slug, photo_id):
    """Serves the pre-generated compressed preview used by the lightbox."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
        return "Not found", 404
    ...
    file_id = photo.get("preview_file_id")
    if not file_id:
        # Fresh-start galleries should have this. This fallback keeps old records
        # viewable until they are removed.
        return photo_thumb(slug, photo_id)
...
def photo_thumb(slug, photo_id):
    """Serves the pre-generated 1280px JPEG thumbnail."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
        return "Not found", 404
    ...
    file_id = photo.get("thumb_file_id")
    if file_id:
        file_path = FILE_PATH_CACHE.get(file_id)
        if not file_path:
            file_path = tg_get_file_path(file_id)
    ...
    # Compatibility fallback for records created before the fresh-start format.
    file_id = photo["file_id"]
    cached = thumb_cache_get(file_id)
    if cached is None:
        file_path = FILE_PATH_CACHE.get(file_id)
        if not file_path:
            file_path = tg_get_file_path(file_id)
        ...
        r.raise_for_status()
        with THUMB_SEMAPHORE:
            cached = make_jpeg_variant(r.content, 1280, 82)
        thumb_cache_set(file_id, cached)

...
def photo_thumb(slug, photo_id):
    ...
    if cached is None:
        ...
        # Only let 2 of these run at once -- decoding a full-resolution photo
        # is memory-heavy, and the free hosting tier doesn't have much RAM.
        with THUMB_SEMAPHORE:
            try:
                img = Image.open(io.BytesIO(r.content))
                # For JPEGs, this decodes at a much smaller size directly
                # instead of loading the full original into memory first --
                # the single biggest fix for out-of-memory crashes here.

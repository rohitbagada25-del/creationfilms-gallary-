...
UPLOAD_CHUNK_SIZE = 2 * 1024 * 1024  # 2 MB resumable chunks
DB_LOCK = threading.RLock()
# Per-upload locks prevent duplicate Telegram/photo creation when a client
# retries a finalize request whose previous response was lost.
UPLOAD_LOCKS = {}
UPLOAD_LOCKS_GUARD = threading.Lock()

def _upload_lock(upload_key):
    with UPLOAD_LOCKS_GUARD:
        lock = UPLOAD_LOCKS.get(upload_key)
        if lock is None:
            lock = threading.RLock()
            UPLOAD_LOCKS[upload_key] = lock
        return lock
...
def _finalize_resumable_photo(slug, upload_key, filename, category, total_size):
    """Turn a fully received temp file into one gallery photo, idempotently."""
    # A finalize request can be retried after a timeout/connection loss.
    # Serialize only requests for this upload key; different photos can still
    # finalize in parallel.
    with _upload_lock(upload_key):
        db = load_db()
        gallery = db["galleries"].get(slug)
        if not gallery:
            raise ValueError("Gallery not found")

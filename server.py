...
# In-memory cache: Telegram file_id -> file_path, so repeat photo/video views
# skip the extra getFile lookup. Cleared automatically whenever the app restarts.
FILE_PATH_CACHE = {}

# In-memory cache: file_id -> resized JPEG bytes. Generated once per photo,
# reused for every grid/lightbox view after that (until the app restarts).
THUMB_CACHE = {}
...
BACKUP_MIN_INTERVAL = 20  # seconds between automatic Telegram backups


def save_db(db, force=False):
    global _last_backup_time, _backup_pending, _backup_in_flight
    with open(DATA_FILE, "w") as f:
        json.dump(db, f, indent=2)

    _backup_pending = True
    now = time.time()
    if not (force or (now - _last_backup_time) > BACKUP_MIN_INTERVAL):
        return  # local file is safe either way; Telegram backup will catch up soon

    if not (BOT_TOKEN and CHANNEL_ID):
        return
    if _backup_in_flight:
        return  # a backup is already running in the background; don't pile up

    def _run_backup(db_snapshot):
        global _last_backup_time, _backup_pending, _backup_in_flight
        _backup_in_flight = True
        try:
            tg_backup_db(db_snapshot)
    ...
    # Run off the request thread -- backing up shouldn't make the person
    # uploading a photo wait for a full JSON round-trip to Telegram.
    threading.Thread(target=_run_backup, args=(json.loads(json.dumps(db)),), daemon=True).start()
...
def tg_send_photo(file_storage, caption=""):
    ...
    for attempt in range(5):
        ...
        if r.status_code == 429:
            # Telegram's flood control -- wait the time it tells us, then retry.
            retry_after = 3
            try:
    ...
    raise Exception("Telegram is rate-limiting uploads right now -- please try again in a moment")
...
@app.route("/")
def root():
    return "Studio gallery server is running.", 200
...
@app.route("/admin/gallery/<slug>/sync", methods=["POST"])
@admin_required
def admin_sync(slug):
    """Forces an immediate Telegram backup -- called once a whole upload
    batch finishes, since save_db() otherwise throttles backups to avoid
    hammering Telegram on every single photo in a 500-1000 photo batch."""
    db = load_db()
    save_db(db, force=True)
    return jsonify({"ok": True})
...
def photo_thumb(slug, photo_id):
    ...
    The original full-resolution file (used for downloads) is untouched --
    this route only affects what gets displayed on screen, which is what
    was making the gallery feel slow to browse.
    """
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
    ...
    if cached is None:
        ...
        try:
            ...
            # If it's not an image Pillow can read, fall back to the original bytes.
            cached = r.content
...
if __name__ == "__main__":
    ...
    # threaded=True is important: without it, this server can only handle ONE
    # request at a time, so an upload in progress blocks every other request
    # (grid thumbnails, other uploads, gallery views) and vice versa -- this
    # was the main cause of uploads appearing to randomly stall or repeat.

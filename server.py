...
def extract_taken_at_from_bytes(data):
    """Read the camera's EXIF DateTimeOriginal/DateTime from an image."""
    try:
        with Image.open(io.BytesIO(data)) as img:
            exif = img.getexif()
            # EXIF 36867 = DateTimeOriginal, 306 = DateTime
...
def tg_download_file_bytes(file_id):
    """Download one Telegram document/image into memory."""
    file_path = tg_get_file_path(file_id)
    r = requests.get(f"{TG_FILE_API}/{file_path}", timeout=120)
    r.raise_for_status()
    return r.content
...
def ensure_taken_dates(slug, gallery):
    """Backfill Taken Date for existing photos once, without changing order."""
    changed = False
    for photo in gallery.get("photos", []):
        if photo.get("taken_at") or not photo.get("file_id"):
            continue
        try:
            data = tg_download_file_bytes(photo["file_id"])
            taken_at = extract_taken_at_from_bytes(data)
            if taken_at:
                photo["taken_at"] = taken_at
                changed = True
        except Exception:
            # Missing/unreadable EXIF must never break gallery loading.
            continue
    if changed:
        with DB_LOCK:
            db = load_db()
            current = db["galleries"].get(slug)
            if current:
                # Only add dates to matching photo IDs, preserving all other data.
                by_id = {p["id"]: p for p in current.get("photos", [])}
                for p in gallery.get("photos", []):
                    if p.get("taken_at") and p["id"] in by_id:
                        by_id[p["id"]]["taken_at"] = p["taken_at"]
                save_db(db)
...
def _finalize_resumable_photo(slug, upload_key, filename, category, total_size):
    ...
    try:
        with open(path, "rb") as fh:
            ...
            f.stream.seek(0)
            original_bytes_for_exif = f.stream.read()
            taken_at = extract_taken_at_from_bytes(original_bytes_for_exif)
            f.stream.seek(0)
...
def api_gallery_photos(slug):
    ...
    # Backfill EXIF Taken Date for existing photos that don't have it yet.
    # This does not reorder or delete anything.
    ensure_taken_dates(slug, gallery)

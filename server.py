...
def photo_thumb(slug, photo_id):
    """Serve a compact thumbnail, using the stored Telegram thumbnail when available."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
    ...
    photo = next((p for p in gallery.get("photos", []) if p.get("id") == photo_id), None)
    if not photo:
        return "Not found", 404
    ...
    if thumb_file_id:
        try:
            file_path = FILE_PATH_CACHE.get(thumb_file_id)
            if not file_path:
                file_path = tg_get_file_path(thumb_file_id)
                FILE_PATH_CACHE[thumb_file_id] = file_path
            ...
            return Response(
                ...
                headers={"Cache-Control": "public, max-age=31536000, immutable",
                         "ETag": f'"{thumb_file_id}"'}
            )
        except Exception as e:
            print(f"WARNING thumbnail Telegram fetch failed for {photo_id}: {e}")

    # Backward-compatible fallback for older photos without thumb_file_id.
    ...
    try:
        with THUMB_SEMAPHORE:
            ...
            if cached is None:
                ...
                r.raise_for_status()
                cached = create_thumbnail_bytes(r.content)
        ...
        print(f"ERROR thumbnail {photo_id}: {e}")
...
def video_proxy(slug, video_id):
    ...
    video = next((v for v in gallery.get("videos", []) if v.get("id") == video_id), None)
    if not video:
        return "Not found", 404
    ...
    try:
        ...
        if not file_path:
            file_path = tg_get_file_path(file_id)
            FILE_PATH_CACHE[file_id] = file_path
        r = requests.get(f"{TG_FILE_API}/{file_path}", stream=True, timeout=300)
        r.raise_for_status()
        return Response(
...
def video_download(slug, video_id):
    ...
    if isinstance(resp, Response):
        db = load_db()
        video = next(
            (v for v in db["galleries"].get(slug, {}).get("videos", [])
...
def generate_missing_thumbnails(slug):
    ...
    def process(photo):
        ...
        return photo["id"], thumb_id

    created = 0
    failed = 0

    # 4 workers gives a large speed-up over the old one-at-a-time migration,
    # while avoiding the RAM spike that caused the Render 503.
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(process, p): p for p in photos}
        for future in as_completed(futures):
            photo = futures[future]
            try:
                photo_id, thumb_id = future.result()
                target = next((p for p in gallery["photos"] if p["id"] == photo_id), None)
                if target:
                    target["thumb_file_id"] = thumb_id
                    created += 1
                    # Save periodically, not once per worker, to keep Telegram
                    # backup traffic reasonable.
                    if created % 10 == 0:
                        save_db(db)
            except Exception as e:
                failed += 1
                print(f"WARNING thumbnail migration failed for {photo.get('id')}: {e}")

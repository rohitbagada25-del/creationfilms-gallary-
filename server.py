...
def tg_send_thumbnail(thumb_bytes, filename, caption=""):
    """Store the small thumbnail in Telegram too, so it survives free-host restarts."""
    files = {"document": (filename, io.BytesIO(thumb_bytes), "image/jpeg")}
    data = {"chat_id": CHANNEL_ID, "caption": caption}
    r = requests.post(f"{TG_API}/sendDocument", data=data, files=files, timeout=60)
    r.raise_for_status()
    result = r.json()["result"]
...
def admin_upload(slug):
    ...
    for f in uploaded:
        ...
        try:
            ...
            # Keep metadata for the original immediately. The generated preview
            # and thumbnail are already local and ready for instant browsing.
            photo = {
...
def photo_preview(slug, photo_id):
    ...
    path = preview_disk_path(slug, photo_id)
    if path.exists():
        return send_file(path, mimetype="image/jpeg", max_age=604800, conditional=True)
    # For this fresh-start version, avoid falling back to the original on a client tap.
    return "Preview unavailable", 404


@app.route("/gallery/<slug>/photo/<photo_id>/thumb")
def photo_thumb(slug, photo_id):
    """Serve the pre-generated 640px thumbnail. Never fetch or resize the original here."""
    db = load_db()
    gallery = db["galleries"].get(slug)
    if not gallery:
        return "Not found", 404

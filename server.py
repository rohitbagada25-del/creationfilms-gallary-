...
def generate_missing_thumbnails(slug):
    ...
    for photo in gallery.get("photos", []):
        try:
            if thumb_cache_get(photo["file_id"]) is not None:
                continue

            # Reuse the same safe, one-at-a-time path used by the public route.
            with THUMB_SEMAPHORE:
                ...
                thumb_cache_set(file_id, out.getvalue())
                created += 1
        except Exception as e:
            failed += 1
...
def photo_thumb(slug, photo_id):
    ...
    # First use a local cached thumbnail. This is the normal fast path.
    ...
    # IMPORTANT: lock BEFORE downloading the full Telegram original.
    # This prevents 68 browser requests from holding 68 large originals in RAM.
    with THUMB_SEMAPHORE:
        cached = thumb_cache_get(file_id)
        if cached is not None:
            return Response(
        ...
        try:
            ...
            # Do not make one bad image break the gallery. Return a controlled
            # error rather than allocating another huge fallback response.

...
UPLOAD_CHUNK_SIZE = 4 * 1024 * 1024  # 1 MB resumable chunks
...
def photo_preview(slug, photo_id):
    ...
    if not photo:
        return "Not found", 404
    # Prefer the locally generated preview for the same fast path as thumbs.
    local_path = preview_disk_path(slug, photo_id)
    if local_path.exists():
        return send_file(local_path, mimetype="image/jpeg", max_age=604800, conditional=True)
    try:
...
def photo_thumb(slug, photo_id):
    ...
    if not photo:
        return "Not found", 404
    # New uploads already have a local thumbnail. Serving it directly avoids
    # an extra Telegram round-trip for every grid image and makes large
    # galleries appear much faster. If the host restarted and the local file
    # is gone, fall back to the persistent Telegram asset.
    local_path = thumb_disk_path(slug, photo_id)
    if local_path.exists():
        return send_file(local_path, mimetype="image/jpeg", max_age=604800, conditional=True)
    try:

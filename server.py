...
try:
    # Newer Python (3.12+) no longer auto-creates an event loop for the main
    # thread. Pyrogram's sync helper module expects one to already exist at
    # import time, so we set one up first -- without this, importing
    # pyrogram crashes with "no current event loop in thread 'MainThread'".
...
# Optional: from https://my.telegram.org -- when set, uploads use Pyrogram's
# MTProto client (the same fast, parallel-chunk protocol Telegram's own apps
# use) instead of the plain Bot API, which is meaningfully faster for large
# photo/video files. Without these, everything still works via the Bot API,
# just slower.
...
# ---------------------------------------------------------------------
# Pyrogram (MTProto) client -- faster uploads to Telegram.
#
# Pyrogram is async, Flask (as run here) is sync, so we run one persistent
# Pyrogram client on its own background thread with its own asyncio event
# loop, and call into it from Flask's sync route handlers via
# asyncio.run_coroutine_threadsafe(...).result(). in_memory=True means no
# session file needs to persist on Render's ephemeral disk -- it logs in
# fresh (using the bot token) every time the process starts.
# ---------------------------------------------------------------------
...
def _pyro_worker():
    global _pyro_client, _pyro_loop
    ...
    try:
        ...
        print(f"WARNING: Pyrogram failed to start ({e}); falling back to the Bot API for uploads.")
        _pyro_ready.set()
        return
    _pyro_ready.set()
    loop.run_forever()
...
def start_pyrogram():
    """Call once at startup. Safe to skip -- everything still works via the
    plain Bot API, just somewhat slower for large files."""
    global PYROGRAM_ENABLED
    if not (PYROGRAM_AVAILABLE and API_ID and API_HASH and BOT_TOKEN):
        return
    threading.Thread(target=_pyro_worker, daemon=True).start()
    _pyro_ready.wait(timeout=30)
    PYROGRAM_ENABLED = _pyro_client is not None


def pyro_run(coro, timeout=600):
    """Run a Pyrogram coroutine from sync Flask code and block for the result."""
    future = asyncio.run_coroutine_threadsafe(coro, _pyro_loop)
...
def tg_send_photo(file_storage, caption=""):
    ...
    When Pyrogram is available and the file is backed by a real path on
    disk (true for the chunked/resumable upload flow), this uploads via
    MTProto directly from that path -- Telegram's own fast, parallel-chunk
    protocol -- instead of a single plain HTTPS POST through the Bot API.
    """
    filename = file_storage.filename or "photo.jpg"
    disk_path = getattr(getattr(file_storage, "stream", None), "name", None)

    if PYROGRAM_ENABLED and disk_path and os.path.exists(str(disk_path)):
        msg = pyro_run(_pyro_client.send_document(
            int(CHANNEL_ID), str(disk_path), file_name=filename, caption=caption,
        ))
        return msg.document.file_id, msg.id

    file_storage.stream.seek(0)
    files = {"document": (filename, file_storage.stream, file_storage.mimetype)}
...
def tg_send_video(file_storage, caption=""):
    ...
    Same Pyrogram-first, Bot-API-fallback approach as tg_send_photo -- videos
    are usually the biggest files uploaded here, so this is where the speed
    ...
    if PYROGRAM_ENABLED and disk_path and os.path.exists(str(disk_path)):
        msg = pyro_run(_pyro_client.send_video(
            int(CHANNEL_ID), str(disk_path), file_name=filename, caption=caption,
            supports_streaming=True,
        ), timeout=1800)
        return msg.video.file_id, msg.id

    file_storage.stream.seek(0)
    files = {"video": (filename, file_storage.stream, file_storage.mimetype or "video/mp4")}
...
def admin_gallery(slug):
    ...
    # Passed to the template so the browser can upload straight to Telegram
    # instead of routing the file through this server -- see the
    # direct-to-Telegram upload code in admin_gallery.html for why.
    return render_template(
        "admin_gallery.html", slug=slug, gallery=gallery,
...
if __name__ == "__main__":
    ...
    start_pyrogram()
    if PYROGRAM_ENABLED:
        print("Pyrogram (MTProto) is active -- uploads will use the fast path.")
    elif API_ID and API_HASH:
        print("WARNING: API_ID/API_HASH set but Pyrogram failed to start -- check the values, falling back to the Bot API.")
    else:
        print("Pyrogram not configured (API_ID/API_HASH missing) -- uploads will use the slower Bot API.")
